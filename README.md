...
```bash
git clone https://github.com/Max-less/AI-Assistant.git
cd AI-Assistant
```
...
## 📦 Шаг 3. Сборка индекса базы знаний (однократно)
...
## 🚀 Шаг 4. Запуск через Docker Compose (рекомендуется)
...
Откройте **три терминала** (индекс из Шага 3 должен быть уже собран).
...
- **`rag_service` падает при старте с «Index file missing»** — не собран индекс. Выполните Шаг 3.
