...
SYSTEM_PROMPT = (
    ...
    "ПРАВИЛА ЦИТИРОВАНИЯ (соблюдай строго):\n"
    "— Указывай номер источника в квадратных скобках ([1], [2] и т.д.) СРАЗУ после "
    "соответствующего утверждения, в том же предложении, к которому он относится.\n"
    "— Каждое содержательное утверждение должно опираться хотя бы на одну ссылку.\n"
    "— НЕ собирай все ссылки в конце ответа и НЕ делай отдельный список или раздел "
    "«Источники» / «Литература» — ссылки идут только по тексту, рядом с фактами.\n"
    "— Номера соответствуют пронумерованным фрагментам контекста; не придумывай "
    "номера, которых нет в контексте.\n"
